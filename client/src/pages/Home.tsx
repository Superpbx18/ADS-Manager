...
type Section = "ภาพรวม" | "Event Stream" | "Data Quality" | "Event Mapping" | "Segments" | "Meta Sync" | "Test Events" | "Campaigns" | "AI แนะนำ" | "A/B Testing" | "Sale Funnel" | "Audience Insights" | "คัดกรองลูกค้า" | "Creative Performance" | "Dayparting" | "GEO จังหวัด" | "Ads manager" | "AI Ad Creator" | "AI Drafts" | "Auto-Pause Rules" | "ตรวจสอบอัตโนมัติ" | "รายงาน" | "Integrations" | "Settings";
...
function Sidebar({ activeSection, setActiveSection, open, close }: { activeSection: Section; setActiveSection: (section: Section) => void; open: boolean; close: () => void }) {
  ...
  const groups = [
    { key: "data", label: "Data & Events", icon: Activity, items: [{ label: "Event Stream", caption: "รับ Event จากระบบที่ 1", icon: Activity }, { label: "Data Quality", caption: "ตรวจคุณภาพและความครบถ้วน", icon: ListFilter }, { label: "Event Mapping", caption: "Map Event ไป Meta", icon: Code2 }, { label: "Test Events", caption: "ตรวจ Payload ก่อนส่ง", icon: ShieldCheck }] },
...
function Overview({ setActiveSection, lastUpdated, onSync }: { setActiveSection: (section: Section) => void; lastUpdated: string; onSync: () => void }) {
  return <>
    <DemoScenario setActiveSection={setActiveSection} />
...
function EventInspectorDrawer({ event, close }: { event: typeof events[number]; close: () => void }) {
  return <div className="drawer-backdrop" onClick={close}><aside className="entity-drawer event-inspector" onClick={event => event.stopPropagation()}><div className="drawer-head"><div><div className="section-kicker">EVENT INSPECTOR</div><h2>{event.name}</h2></div><button className="icon-button" onClick={close}><X size={17} /></button></div><div className="drawer-status-row"><span className={`scenario-dot ${event.tone === "red" ? "coral" : event.tone === "green" ? "green" : "cyan"}`} /><strong>evt_01JXYZ123</strong><span className="analysis-badge cyan">SAMPLE DATA</span></div><div className="inspector-tabs"><button className="active">Raw Event</button><button>Normalized</button><button>Attribution</button><button>Delivery</button></div><div className="drawer-section"><div className="section-kicker">RAW EVENT</div><div className="code-block"><span>{`{`}</span><span><b>"event_name"</b>: "{event.name}",</span><span><b>"event_time"</b>: "2026-09-16T14:28:12+07:00",</span><span><b>"user_id"</b>: "usr_123456",</span><span><b>"value"</b>: 1500,</span><span><b>"currency"</b>: "THB",</span><span><b>"consent_status"</b>: "granted"</span><span>{`}`}</span></div></div><div className="drawer-section"><div className="section-kicker">IDENTITY & CONSENT</div><div className="relationship-list"><span><CheckCircle2 size={13} /> phone normalized + hashed</span><span><CheckCircle2 size={13} /> fbp / fbc present</span><span><CheckCircle2 size={13} /> marketing consent granted</span></div></div><div className="drawer-section"><div className="section-kicker">META DELIVERY</div><div className="drawer-result"><CheckCircle2 size={16} /><div><strong>Delivered · 412 ms</strong><small>Mapped to Purchase · deduplication passed</small></div></div></div><button className="primary-button drawer-cta" onClick={close}>ปิด Inspector <Check size={14} /></button></aside></div>;
}
...
export default function Home() {
  ...
  return <div className="app-shell">
    ...
    <main className="main-area"><Topbar activeSection={activeSection} onMenu={() => setSidebarOpen(true)} onRefresh={refresh} /><div className="content">
      ...
      {["Ads manager", "AI Ad Creator", "AI Drafts", "Auto-Pause Rules", "ตรวจสอบอัตโนมัติ", "รายงาน"].includes(activeSection) && <AdsAutomationPage activeSection={activeSection} />}
